...
FoundationStereo's `core/extractor.py::Feature`:

- Input (typical): `imgs` shaped [2B, 3, H, W], where batch is stacked as:
    imgs = torch.cat([left_batch, right_batch], dim=0)
- Output: dict with key "out":
...
class FeatureAdapter(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        api.eval()
        # st the model for backbone and dpt feature fusion head
        self.api_da3 = api
        ...
        with torch.autocast(
            ...
            if extrinsics is not None:
                with torch.autocast(device_type=imgs.device.type, enabled=False):
                    cam_token = self.cam_enc(extrinsics, intrinsics, imgs.shape[-2:])
            else:
                cam_token = None
